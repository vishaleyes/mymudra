    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="defaultNavbar1">
            <ul class="nav navbar-nav">
                ...
                if (isset(Yii::app()->session[_SITENAME_.'_customer'])) {
                    ?>
                    <li><a href="javascript:void(0);" data-toggle="modal" data-target="#myModalProfile"><?=  Yii::app()->params->msg['_PROFILE_'];?></a></li>
                    <?php
                }
                ?>
...
/*Code start for google social login and register*/
    ...
    $client->setRedirectUri('http://localhost/viia/index.php?r=customer/gplusLogin');
...
?>
        ...
        <!-- Profile Modal -->
        <div class="modal lr-modal fade" id="myModalProfile" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">

                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h5><?=  Yii::app()->params->msg['_PROFILE_'];?></h5>
                    </div>
                    <div class="modal-body">
                        <div>
                            ...
                            <div class="tab-content">
                                <div class="alert alert-danger <?php
                                if(!isset(Yii::app()->session[_SITENAME_.'_customer_reg_err_msg'])){ echo "hide";}
                                ?>" id="edit_profile_err_msg_div" style="margin-bottom:0px;margin-top:10px;">
                                    <button class="close" data-close="alert"></button>
                                    <span id="edit_profile_err_msg"><?php
                                        if((isset(Yii::app()->session[_SITENAME_.'_customer_reg_err_msg']) && Yii::app()->session[_SITENAME_.'_customer_reg_err_msg']!='')){ echo Yii::app()->session[_SITENAME_.'_customer_reg_err_msg'];}
                                        ?></span>
                                </div>
                                <div class="alert alert-success hide" id="edit_profile_success_msg_div" style="margin-bottom:0px;margin-top:10px;">
                                    <button class="close" data-close="alert"></button>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                            <form class="l-r" action="<?php echo Yii::app()->params->base_path;?>customer/editProfile" method="post" name="edit_profile_form" id="edit_profile_form">
                                                <div class="form-group">
                                                    <label for="exampleInputEmail1" class="control-label"><?= Yii::app()->params->msg['_FIRST_NAME_'];?></label>
                                                    <input type="text" class="form-control" id="first_name" name="first_name" placeholder="<?=  Yii::app()->params->msg['_FIRST_NAME_'];?>" value="<?php echo Yii::app()->session[_SITENAME_.'_customer_first_name'];?>">
                                                </div>

                                                <div class="form-group">
                                                    <label for="exampleInputEmail1" class="control-label"><?= Yii::app()->params->msg['_LAST_NAME_'];?></label>
                                                    <input type="text" class="form-control" id="last_name" name="last_name" placeholder="<?=  Yii::app()->params->msg['_LAST_NAME_'];?>" value="<?php echo Yii::app()->session[_SITENAME_.'_customer_last_name'];?>">
                                                </div>
                                                <div class="form-group">
                                                    <label for="exampleInputEmail1" class="control-label"><?=  Yii::app()->params->msg['_EMAIL_'];?></label>
                                                    <input type="email" class="form-control" id="new_password_confirm" name="new_password_confirm" placeholder="<?=  Yii::app()->params->msg['_EMAIL_'];?>" value="<?php echo Yii::app()->session[_SITENAME_.'_customer_email'];?>" disabled>
                                                </div>
                                                <div class="form-group">
                                                    <label for="exampleInputEmail1" class="control-label"><?=  Yii::app()->params->msg['_USERNAME_'];?></label>
                                                    <input type="text" class="form-control" id="new_password_confirm" name="new_password_confirm" placeholder="<?=  Yii::app()->params->msg['_USERNAME_'];?>" value="<?php echo Yii::app()->session[_SITENAME_.'_customer_username'];?>" disabled>
                                                </div>
                                                <div class="form-group">
                                                    <label for="exampleInputEmail1" class="control-label"><?=  Yii::app()->params->msg['_MOBILE_NUMBER_'];?></label>
                                                    <input type="text" class="form-control" id="mobile_number" name="mobile_number" placeholder="<?= Yii::app()->params->msg['_MOBILE_NUMBER_'];?>" value="<?= Yii::app()->session[_SITENAME_.'_customer_mobile_number'];?>">
                                                </div>

                                                <div class="clearfix"></div>

                                                <div class="login-btn">
                                                    <button type="submit" class="btn btn-default"  id="submit_reset_password" name="submit_reset_password" ><?=  Yii::app()->params->msg['_EDIT_'];?> <img style="display:none;float: left; position: absolute; margin-top: -34px; width: 20%;" src="<?php echo Yii::app()->params->base_url;?>/assets/img/loading_dots.gif" id="Loaderaction3"></button>
                                                </div>
        ...
        <footer>
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <p class="text-center copyright">© <?php echo date("Y"); ?> <span><?=  Yii::app()->params->msg['_VIIA_'];?></span>.  <?=  Yii::app()->params->msg['_ALL_RESERVED_'];?></p>
                    </div>
                </div>
            </div>
        </footer>
        ...
        <script>
            ...
            function customerLogin(){
                ...
                $.ajax({
                    ...
                    {
                        if(data == 'true'){
                            window.location = '<?php echo Yii::app()->request->url; ?>';
                        }
                        else
                        {
            ...
            function customerEditProfile(){
                ...
                $("#edit_profile_form").css('opacity', '0.50');
                //alert(keyword);
                $.ajax({
                    type: 'POST',
                    url: '<?php echo Yii::app()->params->base_path; ?>customer/editProfile',
                    ...
                    {
                        if(data.status == '1'){
                            $('#edit_profile_success_msg').html('Profile successfully edited.');
                            $('#edit_profile_success_msg_div').removeClass('hide')
                            setTimeout(function()
                            {
                                $('#edit_profile_success_msg_div').addClass('hide');
                        ...
                        {
                            $('#edit_profile_err_msg').html(data.message);
                            $('#edit_profile_err_msg_div').removeClass('hide')
                            setTimeout(function()
                            {
                                $('#edit_profile_err_msg_div').addClass('hide');
                            }, 6000 );
                        }

                        $("#edit_profile_form").css('opacity', '1');
                    }
                });
            }
                ...
                $('#register_form').validate({
                    ...
                    messages: {
                        ...
                        mobile_number:{
                            ...
                            minlength: "<?=  Yii::app()->params->msg['_PHONE_MIN_LENGTH_'];?>",
                            maxlength: "<?=  Yii::app()->params->msg['_PHONE_MAX_LENGTH_'];?>",
                            number: "<?=  Yii::app()->params->msg['_VALID_NUMBER_'];?>",
                        },
                        terms_condition:{
