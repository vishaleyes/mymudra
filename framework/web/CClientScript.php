...
class CClientScript extends CApplicationComponent
{
	 ...
	 * will refer to the file 'protected/comments.js'.
	 *
	 ...
	 * then the 'comments.js' script will be rendered using URL '/demo/comments.js'.
	 *
	...
	public function render(&$output)
	{
		...
		if(!isset(Yii::app()->session['prefferd_language']))
		{
			Yii::app()->session['prefferd_language']=DEFAULT_LANGUAGE;
		}
		
		$smarty = new smartyML(Yii::app()->session['prefferd_language']);
		$output=smarty_prefilter_i18n($output,$smarty);
		if(!$this->hasScripts)
			return;
