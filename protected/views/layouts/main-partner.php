        ...
        function initMenu()
        {
            $(document).ready(function() {
                $('li.nav').click(function() {
                    ...
                    if($arr == '' && $(this).children('a').attr("rel"))
                    {
...
<!-- BEGIN HEADER -->
<div class="page-header navbar navbar-fixed-top" style="background: #337ab7;!important;">
    <!-- BEGIN HEADER INNER -->
    <div class="page-header-inner ">
        <!-- BEGIN LOGO -->
        <div class="page-logo">
            <a href="index.html">
                <img src="assets/pages/img/login_logo.png" alt="logo" class="logo-default" style="height: 25px;
width: 110px;" /> </a>
                ...
                <ul class="nav navbar-nav pull-right">
                   ...
                   <!-- <li class="dropdown dropdown-extended dropdown-tasks dropdown-dark" id="header_task_bar">
                    ...
                    <li class="dropdown dropdown-user dropdown-dark">
                        <a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
                            <span class="username username-hide-on-mobile"> <?php echo Yii::app()->session[_SITENAME_.'_partner_name']; ?> </span>
                        ...
                        <ul class="dropdown-menu dropdown-menu-default">
                          <!--  <li class="nav" id="profile" lang="profile">
                            ...
                            <li class="nav" id="changePassword" lang="changePassword">
                                <a href="<?php echo Yii::app()->params->base_path ; ?>partner/changePassword" lang="">
                            ...
                            <li class="divider"> </li>
                            <li>
                                <a href="<?php echo Yii::app()->params->base_path ; ?>partner/Logout">
...
<div class="page-container">
    ...
    <div class="page-sidebar-wrapper">
        ...
        <div class="page-sidebar navbar-collapse collapse">
            ...
            <?php
            $TblAdminMenuObj = new TblAdminMenu();
            $menus = $TblAdminMenuObj->getAllMenus();

            $TblRolePermissionObj = new TblRolePermission();
            $menu_ids = $TblRolePermissionObj->getMenusByRoleID(Yii::app()->session[_SITENAME_.'_partner_role']);

            $menu_arr = explode(",", $menu_ids['menu_ids']);

            $TblAdminMenuObj = new TblAdminMenu();
            $masterSubMenus = $TblAdminMenuObj->getMasterMenusArr();

            $menu_arr = explode(",", $menu_ids['menu_ids']);
            $master_sub_menu_arr = explode(",", $masterSubMenus);
            //array_push($master_sub_menu_arr, "Reports");
                ...
                <?php foreach ($menus as $row) { ?>
                    ...
                    <?php if ($row['menu_name'] == "Dashboard" && in_array($row['menu_id'], $menu_arr)) {?>
                        <li <?php if (isset(Yii::app()->session['current']) && Yii::app()->session['current'] == $row['menu_name']) { ?> class="nav-item start active open" <?php } ?>>
                            <a href="<?php echo Yii::app()->params->base_path; ?><?php echo "partner/".$row['page_url']; ?>"
                    ...
                    <?php } else if(($row['menu_name'] == "Reports" || $row['is_master'] == 1 )  && in_array($row['menu_id'], $menu_arr )){ ?>
                        <li <?php if (in_array( Yii::app()->session['current'], $master_sub_menu_arr)) { ?> class="nav-item nav start active open" <?php } ?>>
                            ...
                            <ul class="sub-menu">
                                <?php
                                $TblMenuObj = new TblAdminMenu();
                                $subMenu = $TblMenuObj->getMasterMenus();
                                ?>
                                <?php foreach ($subMenu as $sub) {  ?>
                                    <li <?php if (isset(Yii::app()->session['current']) && Yii::app()->session['current'] == $sub['menu_name']) { ?> class="active nav nav-item start" <?php } else { ?> class="nav nav-item start" <?php } ?>
                                            id="<?php echo $sub['menu_name']; ?>" lang="<?php echo $sub['menu_name']; ?>">
                                        <a href="<?php echo Yii::app()->params->base_path; ?><?php echo "partner/".$sub['page_url']; ?>/menu_id/<?php echo $sub['menu_id']; ?>" lang=""
                                           class="nav-link nav-toggle">
                                            <i class="<?php echo $sub['menu_icon']; ?>"></i>
                                            <span class="title"><?php echo $sub['menu_name']; ?></span>
                    ...
                    <?php } else { ?>
                        <?php if(in_array($row['menu_id'], $menu_arr)) { ?>
                            <li <?php if (isset(Yii::app()->session['current']) && Yii::app()->session['current'] == $row['current_session']) { ?> class="active nav nav-item start" <?php } else { ?> class="nav nav-item start" <?php } ?>
                                    id="<?php echo $row['menu_name']; ?>" lang="<?php echo $row['menu_name']; ?>">
                                <a href="<?php echo Yii::app()->params->base_path; ?><?php echo "partner/".$row['page_url']; ?>/menu_id/<?php echo $row['menu_id']; ?>"  lang="" class="nav-link nav-toggle">
                                    <i class="<?php echo $row['menu_icon']; ?>"></i>
                                    <span class="title"><?php echo $row['menu_name']; ?></span>
...
<div class="page-footer">
    <div class="page-footer-inner"> <?php echo date("Y"); ?> &copy; ViiA  |&nbsp;
